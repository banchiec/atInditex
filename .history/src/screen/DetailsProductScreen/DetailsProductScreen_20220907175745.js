...
const DetailsProductScreen = () => {
  ...
  useEffect(() => {
    if(product){
      setSelector({
        name: 'colors',
        valuesColors: product.colors,
      })
    }
  },[product])

  const handleChangeSelector = (e) => {
    e.target.name === 'colors'? setColors(true): setColors(false)
    e.target.name === 'storage'? setStorage(true): setStorage(false)
        ...
        <ContainerDetails>
          <Container>
            ...
            <ContainerDescription>
              ...
              <Actions>
                <ContainerOptions>
                  { selector.name === 'colors' ? (selector?.valuesColors?.map((item) => {
                    return(
                      <ButtonOptions
                        key={item} 
                    ...
                    selectorStorage?.valuesStorage?.map((item) => {
                      return(
                        <ButtonOptions
                          key={item}
                          name={item}
