...
const DetailsProductScreen = () => {
  ...
  const handleChangeSelector = (e) => {
    ...
    e.target.name === 'storage'? setSelector( {name:'storage', values:product?.internalMemory}): setSelector({name: 'colors', values: product?.colors})
  }
              ...
              <Actions>
                <ContainerOptions>
                  {selector?.values?.map((item) => {
                    return(
                      <ButtonDescription key={item} colorBackGround={`${item}`}>{item}</ButtonDescription>
                    )
                  })}
                </ContainerOptions>
                <ContainerSelector>
